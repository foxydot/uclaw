<?php 
function nightcourt_preprocess_ddblock_cycle_block_content(&$vars){
	//ts_data($vars);
...
function nightcourt_preprocess_ddblock_cycle_pager_content(&$vars){
    if ($vars['pager_settings']['view_name'] == 'news') {
      if (!empty($vars['content'])) {
        foreach ($vars['content'] as $key1 => $result) {
	      ...
	      $pager_items[$key1]['text'] = nightcourt_trim_headline($result->node_revisions_teaser, 20);
...
function nightcourt_trim_headline($text, $length = 35) {
