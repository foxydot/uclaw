...
if (theme_get_setting('mcbeal_rebuild_registry')) {
  drupal_rebuild_theme_registry();
}

// Add Zen Tabs styles
if (theme_get_setting('mcbeal_zen_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'mcbeal') .'/css/tabs.css', 'theme', 'screen');
}
...
function mcbeal_preprocess_page(&$vars, $hook) {
  $vars['edit'] = mcbeal_check_edit($vars);
  $node = $vars['node'];
	...
	if($node->field_news[0]['value'] || !empty($faculty_news)){
  	...
  	case "homepage":
  		if(!empty($node->field_feature_img[0]['view'])){
	  		$vars['header_feature'][1]['feature_img'] = $node->field_feature_img[0]['filepath'];
  		...
  		if(!empty($node->field_feature_img_2[0]['view'])){
	  		$vars['header_feature'][2]['feature_img'] = $node->field_feature_img_2[0]['filepath'];
  		...
  		if(!empty($node->field_feature_img_3[0]['view'])){
	  		$vars['header_feature'][3]['feature_img'] = $node->field_feature_img_3[0]['filepath'];
  		...
  		if(!empty($node->field_feature_img_4[0]['view'])){
	  		$vars['header_feature'][4]['feature_img'] = $node->field_feature_img_4[0]['filepath'];
  		...
  		if(!empty($node->field_feature_img_5[0]['view'])){
	  		$vars['header_feature'][5]['feature_img'] = $node->field_feature_img_5[0]['filepath'];
  ...
  if (user_access('administer blocks')) {
	...
	if (theme_get_setting('mcbeal_wireframe')) {
  ...
  if (!$vars['is_front']) {
    ...
    $body_classes[] = mcbeal_id_safe('page-'. $path);
    $body_classes[] = mcbeal_id_safe('section-'. $section);
...
function mcbeal_check_edit(&$vars){
...
function mcbeal_preprocess_node(&$vars, $hook) {
  ...
  $classes[] = mcbeal_id_safe('node-type-' . $vars['type']);
...
function mcbeal_preprocess_block(&$vars, $hook) {
    $block = $vars['block'];
   ...
   // $vars['content'] = preg_replace("/".preg_quote($vars['attachments'],'/')."/",'',$vars['content']);
    ...
    $classes = array('block');
    $classes[] = mcbeal_id_safe('block-' . $vars['block']->module);
    $classes[] = mcbeal_id_safe('block-' . $vars['block']->region);
    $classes[] = mcbeal_id_safe('block-id-' . $vars['block']->bid);
    $classes[] = 'clearfix';
    ...
    //if (theme_get_setting('mcbeal_block_editing') && user_access('administer blocks')) {
    if (user_access('administer blocks')) {
...
function mcbeal_preprocess_comment(&$vars, $hook) {
...
function mcbeal_menu_item_link($link) {
...
function mcbeal_menu_local_tasks() {
  $output = '';
...
function mcbeal_menu_item($link, $has_children, $menu = '', $in_active_trail = FALSE, $extra_class = NULL) {
...
#New line added to get unique classes for each menu item
  $css_class = mcbeal_id_safe(str_replace(' ', '_', strip_tags($link)));
...
function mcbeal_id_safe($string) {
...
function mcbeal_breadcrumb($breadcrumb) {
  if (!empty($breadcrumb)) {
...
*/
function mcbeal_preprocess_search_theme_form(&$vars, $hook) {
...
function mcbeal_get_cache_url($imgtag){
